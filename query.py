...
def args_parser(msg: str) -> argparse.Namespace:
    ...
    parser.add_argument("-pg", action = 'store_true', help = "Optional argument: Write the .json output file in a new postgres 4 database when option -json is used.")
...
def main():
    ...
    json_to_db = arguments.get('db')
    json_to_db_pg = arguments.get('pg')
